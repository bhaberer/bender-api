...
# Module for conveniance methods used in multiple plugins.
module Helpers
  ...
  def self.build_time_hash(secs, units)
    hash = { days: (secs / 86_400).floor,
             hours: ((secs % 86_400) / 3600).floor,
             mins: ((secs % 3_600) / 60).floor,
             seconds: (secs % 60).floor }
    hash.delete_if { |period, _time| units && !units.include?(period) }
    hash
  end
